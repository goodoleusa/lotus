...
#### [v0.6.0](https://github.com/BugBlocker/lotus/releases/tag/v0.6.0)

> 21 January 2026

##### Chainable Fluent API

This release introduces a powerful **fluent/chainable API** for writing expressive security scripts:

- **`str()`** - Chainable string operations
  - Transforms: `upper()`, `lower()`, `trim()`, `reverse()`, `replace()`, `append()`, `prepend()`
  - Regex: `match()`, `find()`, `find_all()`, `replace_regex()`
  - Encoding: `url_encode()`, `url_decode()`, `base64_encode()`, `base64_decode()`, `html_encode()`, `html_decode()`
  - Predicates: `contains()`, `startswith()`, `endswith()`, `is_empty()`
  - Split: `split()` returns chainable table

- **`html()`** - Chainable HTML/CSS selector parsing
  - `select()`, `select_one()`, `attr()`, `text()`, `xss_selector()`
  - `first()`, `last()`, `get()`, `each()`

- **`json()`** - Chainable JSON navigation with dot notation
  - `get("path.to.value")`, `has()`, `keys()`
  - Type checks: `is_null()`, `is_string()`, `is_array()`, `is_object()`
  - `each()` for array iteration

- **`tbl()`** - Chainable table/array operations
  - `map()`, `filter()`, `each()`, `find()`, `any()`, `all()`
  - `sort()`, `unique()`, `reverse()`, `join()`, `take()`, `skip()`

##### New HttpMessage Methods
- `get_param(name)` - Get single URL parameter value
- `get_params()` - Get all parameters as table
- `has_param(name)` - Check if parameter exists
- `host()` - Get hostname
- `scheme()` - Get URL scheme (http/https)
- `port()` - Get port number
- `set_path(path)` - Modify URL path

##### New Response Helper Methods
- `resp:json()` - Parse response body as JSON
- `resp:status_ok()` - Check if status is 2xx
- `resp:has_header(name)` - Case-insensitive header check
- `resp:get_header(name)` - Get header value

##### Global Utilities
- `sleep(seconds)` - Pause execution
- `randstr(len)` - Generate random alphanumeric string

##### Backwards Compatibility
All legacy functions (`str_contains`, `json_encode`, `html_search`, `is_match`, etc.) remain available.
